...
export default {
  ...
  // 我们一般是在beforeMount取处理数据，处理完成后后期就可以使用
  beforeMount() {
  ...
  methods: {
    ...
    async addShopCart() {
      ...
      // 根据添加成功还是失败来决定要不要跳转搭配添加购物车成功页面
      let { skuId, skuNum } = this;
      // dispatch 分发触发 调用相应的请求函数
      //this.$store.dispatch('addorUpdateCart',{skuId,skuNum})就是函数表达式
      // actions当中异步函数的返回值，异步函数返回值一定是promise
      try {
        ...
        // localStorage setItemg getItem removeItem clear
        // sessionStorage setItemg getItem removeItem clear

        sessionStorage.setItem("SKUINFO_KEY", JSON.stringify(this.skuInfo));
        this.$router.push("/addcartsucess?skuNum=" + this.skuNum);
      } catch (error) {
        alert(error.message);
      }
      /* 
